 ...
 *
 * SDR always. HDR10, HDR10+ and HLG only when the display reports them, so a server that can
 * preserve HDR through a re-encode tone-maps for an SDR screen instead. Dolby Vision types only
 * for the profiles a decoder advertises for [codec]; without them the server strips the Dolby
 * Vision metadata from a compatible base layer rather than sending a stream nothing here decodes.
 */
